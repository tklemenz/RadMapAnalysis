...
void plotClusterEvent(const char *inputFile, const char *outputFile, ULong_t procNr)
{
  ...
  TH2D* hMulti3L12tot  = new TH2D("hMulti3L12tot","Multiplicity 3 - Layer 1&2;ToT L1;ToT L2",100,0,35,100,0,35);
  TH2D* hMulti3L13tot  = new TH2D("hMulti3L13tot","Multiplicity 3 - Layer 1&3;ToT L1;ToT L3",100,0,35,100,0,35);
  TH2D* hMulti3L14tot  = new TH2D("hMulti3L14tot","Multiplicity 3 - Layer 1&4;ToT L1;ToT L4",100,0,35,100,0,35);
  TH2D* hMulti3L23tot  = new TH2D("hMulti3L23tot","Multiplicity 3 - Layer 2&3;ToT L2;ToT L3",100,0,35,100,0,35);
  TH2D* hMulti3L24tot  = new TH2D("hMulti3L24tot","Multiplicity 3 - Layer 2&4;ToT L2;ToT L4",100,0,35,100,0,35);
  TH2D* hMulti3L34tot  = new TH2D("hMulti3L34tot","Multiplicity 3 - Layer 3&4;ToT L3;ToT L4",100,0,35,100,0,35);

  TH2D* hMulti4L12tot  = new TH2D("hMulti4L12tot","Multiplicity 4 - Layer 1&2;ToT L1;ToT L2",100,0,35,100,0,35);
  TH2D* hMulti4L13tot  = new TH2D("hMulti4L13tot","Multiplicity 4 - Layer 1&3;ToT L1;ToT L3",100,0,35,100,0,35);
  TH2D* hMulti4L14tot  = new TH2D("hMulti4L14tot","Multiplicity 4 - Layer 1&4;ToT L1;ToT L4",100,0,35,100,0,35);
  TH2D* hMulti4L23tot  = new TH2D("hMulti4L23tot","Multiplicity 4 - Layer 2&3;ToT L2;ToT L3",100,0,35,100,0,35);
  TH2D* hMulti4L24tot  = new TH2D("hMulti4L24tot","Multiplicity 4 - Layer 2&4;ToT L2;ToT L4",100,0,35,100,0,35);
  TH2D* hMulti4L34tot  = new TH2D("hMulti4L34tot","Multiplicity 4 - Layer 3&4;ToT L3;ToT L4",100,0,35,100,0,35);

  TH2D* hAllMultiL12tot  = new TH2D("hAllMultiL12tot","all Multiplicities - Layer 1&2;ToT L1;ToT L2",100,0,35,100,0,35);
  TH2D* hAllMultiL13tot  = new TH2D("hAllMultiL13tot","all Multiplicities - Layer 1&3;ToT L1;ToT L3",100,0,35,100,0,35);
  TH2D* hAllMultiL14tot  = new TH2D("hAllMultiL14tot","all Multiplicities - Layer 1&4;ToT L1;ToT L4",100,0,35,100,0,35);
  TH2D* hAllMultiL23tot  = new TH2D("hAllMultiL23tot","all Multiplicities - Layer 2&3;ToT L2;ToT L3",100,0,35,100,0,35);
  TH2D* hAllMultiL24tot  = new TH2D("hAllMultiL24tot","all Multiplicities - Layer 2&4;ToT L2;ToT L4",100,0,35,100,0,35);
  TH2D* hAllMultiL34tot  = new TH2D("hAllMultiL34tot","all Multiplicities - Layer 3&4;ToT L3;ToT L4",100,0,35,100,0,35);
  ...
  for (ULong_t entry = 0; entry < nEvents; entry++) {
    ...
    if(buffer==3) {
      hMulti3L12tot->Fill(qToTBuffer.at(0).size()!=0 ? qToTBuffer.at(0).at(0) : 0, qToTBuffer.at(1).size()!=0 ? qToTBuffer.at(1).at(0) : 0);
      hMulti3L13tot->Fill(qToTBuffer.at(0).size()!=0 ? qToTBuffer.at(0).at(0) : 0, qToTBuffer.at(2).size()!=0 ? qToTBuffer.at(2).at(0) : 0);
      hMulti3L14tot->Fill(qToTBuffer.at(0).size()!=0 ? qToTBuffer.at(0).at(0) : 0, qToTBuffer.at(3).size()!=0 ? qToTBuffer.at(3).at(0) : 0);
      hMulti3L23tot->Fill(qToTBuffer.at(1).size()!=0 ? qToTBuffer.at(1).at(0) : 0, qToTBuffer.at(2).size()!=0 ? qToTBuffer.at(2).at(0) : 0);
      hMulti3L24tot->Fill(qToTBuffer.at(1).size()!=0 ? qToTBuffer.at(1).at(0) : 0, qToTBuffer.at(3).size()!=0 ? qToTBuffer.at(3).at(0) : 0);
      hMulti3L34tot->Fill(qToTBuffer.at(2).size()!=0 ? qToTBuffer.at(2).at(0) : 0, qToTBuffer.at(3).size()!=0 ? qToTBuffer.at(3).at(0) : 0);
    }

    if(buffer==4) {
      hMulti4L12tot->Fill(qToTBuffer.at(0).at(0), qToTBuffer.at(1).at(0));
      hMulti4L13tot->Fill(qToTBuffer.at(0).at(0), qToTBuffer.at(2).at(0));
      hMulti4L14tot->Fill(qToTBuffer.at(0).at(0), qToTBuffer.at(3).at(0));
      hMulti4L23tot->Fill(qToTBuffer.at(1).at(0), qToTBuffer.at(2).at(0));
      hMulti4L24tot->Fill(qToTBuffer.at(1).at(0), qToTBuffer.at(3).at(0));
      hMulti4L34tot->Fill(qToTBuffer.at(2).at(0), qToTBuffer.at(3).at(0));
    }

    hAllMultiL12tot->Fill(qToTBuffer.at(0).size()!=0 ? qToTBuffer.at(0).at(0) : 0, qToTBuffer.at(1).size()!=0 ? qToTBuffer.at(1).at(0) : 0);
    hAllMultiL13tot->Fill(qToTBuffer.at(0).size()!=0 ? qToTBuffer.at(0).at(0) : 0, qToTBuffer.at(2).size()!=0 ? qToTBuffer.at(2).at(0) : 0);
    hAllMultiL14tot->Fill(qToTBuffer.at(0).size()!=0 ? qToTBuffer.at(0).at(0) : 0, qToTBuffer.at(3).size()!=0 ? qToTBuffer.at(3).at(0) : 0);
    hAllMultiL23tot->Fill(qToTBuffer.at(1).size()!=0 ? qToTBuffer.at(1).at(0) : 0, qToTBuffer.at(2).size()!=0 ? qToTBuffer.at(2).at(0) : 0);
    hAllMultiL24tot->Fill(qToTBuffer.at(1).size()!=0 ? qToTBuffer.at(1).at(0) : 0, qToTBuffer.at(3).size()!=0 ? qToTBuffer.at(3).at(0) : 0);
    hAllMultiL34tot->Fill(qToTBuffer.at(2).size()!=0 ? qToTBuffer.at(2).at(0) : 0, qToTBuffer.at(3).size()!=0 ? qToTBuffer.at(3).at(0) : 0);
  ...
  TCanvas *c6 = new TCanvas("c6","M4ToT", 1500, 700);
  c6->Divide(3,2);
  c6->cd(1);
  //gPad->SetLogz();
  hMulti4L12tot->Draw("COLZ");
  c6->cd(2);
  //gPad->SetLogz();
  hMulti4L13tot->Draw("COLZ");
  c6->cd(3);
  //gPad->SetLogz();
  hMulti4L14tot->Draw("COLZ");
  c6->cd(4);
  //gPad->SetLogz();
  hMulti4L23tot->Draw("COLZ");
  c6->cd(5);
  //gPad->SetLogz();
  hMulti4L24tot->Draw("COLZ");
  c6->cd(6);
  //gPad->SetLogz();
  hMulti4L34tot->Draw("COLZ");

  TCanvas *c7 = new TCanvas("c7","AllMToT", 1500, 700);
  c7->Divide(3,2);
  c7->cd(1);
  //gPad->SetLogz();
  hAllMultiL12tot->Draw("COLZ");
  c7->cd(2);
  //gPad->SetLogz();
  hAllMultiL13tot->Draw("COLZ");
  c7->cd(3);
  //gPad->SetLogz();
  hAllMultiL14tot->Draw("COLZ");
  c7->cd(4);
  //gPad->SetLogz();
  hAllMultiL23tot->Draw("COLZ");
  c7->cd(5);
  //gPad->SetLogz();
  hAllMultiL24tot->Draw("COLZ");
  c7->cd(6);
  //gPad->SetLogz();
  hAllMultiL34tot->Draw("COLZ");

  TCanvas *c8 = new TCanvas("c8","M3ToT", 1500, 700);
  c8->Divide(3,2);
  c8->cd(1);
  //gPad->SetLogz();
  hMulti3L12tot->Draw("COLZ");
  c8->cd(2);
  //gPad->SetLogz();
  hMulti3L13tot->Draw("COLZ");
  c8->cd(3);
  //gPad->SetLogz();
  hMulti3L14tot->Draw("COLZ");
  c8->cd(4);
  //gPad->SetLogz();
  hMulti3L23tot->Draw("COLZ");
  c8->cd(5);
  //gPad->SetLogz();
  hMulti3L24tot->Draw("COLZ");
  c8->cd(6);
  //gPad->SetLogz();
  hMulti3L34tot->Draw("COLZ");
  ...
  fout->WriteObject(c8, "Multi3ToTcanvas");
  fout->WriteObject(c6, "Multi4ToTcanvas");
  fout->WriteObject(c7, "AllMultiToTcanvas");
